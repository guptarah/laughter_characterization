...
%    assert(mod(length(ParametersToOpt),2)==1,'Number of initial parameters must be odd');
    M = (length(ParametersConstant))
    Lambda0 = ParametersToOpt(1);
    Lambda = ParametersToOpt(2);
    Alpha = ParametersToOpt(3);
    YPoints = Lambda0*ones(1,length(XPoints));
    for m=1:M
        IndicesInUse = find(XPoints>ParametersConstant(m));
        YPoints(IndicesInUse) = YPoints(IndicesInUse)+Lambda*exp(-Alpha*(XPoints(IndicesInUse)-ParametersConstant(m)));
    end
...
%    assert(mod(length(ParametersToOpt),2)==1,'Number of initial parameters must be odd');
    ...
    for m=1:M
        ...
        if isempty(IndicesInUse)
            ...
            Window = Lambda*tukeywin(round(Width),TukeyWinRatio)';
...
%    assert(mod(length(ParametersToOpt),2)==1,'Number of initial parameters must be odd');
    ...
    for m=1:M
        ...
        if isempty(IndicesInUse)
            ...
            Window = Lambda*rectwin(round(Width))';
            if length(IndicesInUse)>Window
                %if length(YPoints)>IndicesInUse(1)+length(Window)-10
                %    	YPoints(IndicesInUse(1):end) = YPoints(IndicesInUse(1):end)+Window(1:length(YPoints)-IndicesInUse(1)+1);
		%else
                	YPoints(IndicesInUse(1):IndicesInUse(1)+length(Window)-1) = YPoints(IndicesInUse(1):IndicesInUse(1)+length(Window)-1)+Window;
		%end
            else
